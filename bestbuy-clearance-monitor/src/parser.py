...
def normalize_online_availability(raw_text: str) -> str:
    ...
    text = raw_text.lower()
    if any(k in text for k in ("sold out online", "out of stock online", "unavailable online")):
        return "sold_out_online"
    if any(k in text for k in ("not available for delivery", "delivery unavailable")):
        return "not_available_for_delivery"
    if any(k in text for k in ("available to ship", "available online", "get it shipped", "in stock online")):
...
def normalize_pickup_availability(raw_text: str) -> str:
    ...
    text = raw_text.lower()
    if any(k in text for k in ("not available for pickup", "pickup unavailable", "not available at")):
        return "not_available_for_pickup"
    if any(k in text for k in ("available for pickup", "ready for pickup", "pick up as soon as")):
...
def parse_product_page(url: str, html: str) -> dict[str, str]:
    ...
    delivery_raw = _find_text_by_keywords(
        soup,
        ("available to ship", "sold out online", "not available for delivery", "delivery"),
    )
    pickup_raw = _find_text_by_keywords(
        soup,
        ("available for pickup", "not available for pickup", "pickup"),
    )
